...
export default function PageComponent({ ordersWithProducts }: Props) {
  ...
  const orderedProducts = ordersWithProducts.flatMap(
    (order: OrdersWithProducts[number]) =>
      order.order_items.map(
        (item: { product: OrderedProducts[number]["product"] }) => ({
          order_id: order.id,
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <Table>
        ...
        <TableBody>
          {ordersWithProducts.map((order: OrdersWithProducts[number]) => (

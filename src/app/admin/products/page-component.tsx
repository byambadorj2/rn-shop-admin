...
import { Category } from "@/app/admin/categories/catergories.types";
import {
...
export const ProductPageComponent: FC<Props> = ({
  ...
  const productCreateUpdateHandler = async (
    ...
    const uploadFile = async (file: File): Promise<string> => {
      // Implement the file upload logic here and return the URL as a string
      return "uploaded-file-url"; // Replace this with actual upload logic
    };
    ...
    if (images.length > 0) {
      ...
      try {
        imageUrls = (await Promise.all(imagesPromises)) as unknown as string[];
      } catch (error) {
